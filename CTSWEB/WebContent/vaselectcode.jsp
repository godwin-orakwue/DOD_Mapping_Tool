...
<f:view>
	...
	<a4j:form id="test1">
		...
		Filter Text: <h:inputText id="FILTERTEXT" value="#{backing_tab.vafilterText}" /><a4j:commandButton value="Filter"
                                    action="#{backing_tab.vafilter}" 
                                                   id="filter" styleClass="rich-button default" />
		<h:inputHidden id="mapname" value="#{backing_tab.mapName}" />
		<rich:panel>
			<h:selectOneListbox id="valuemenu" value="#{backing_tab.returncode}" size="30" >
                <f:selectItems value="#{backing_tab.valookupValue}" /> 
			</h:selectOneListbox>
		</rich:panel>
		<a4j:commandButton id="return2" action="#{backing_tab.valookupReturn}" value="Return"
